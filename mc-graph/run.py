...
from utils import datagen, preprocess_dataset, clustering_accuracy, clustering_f1_score
...
print('-----------------', dataset, '-----------------')
A, X, labels, n_classes = datagen(dataset)
A, X, labels = preprocess_dataset(A, X, labels)
...
for run in range(runs):
    ...
    Z, F, XW, losses = mc(H, n_classes=n_classes, proj_dim=min(n_classes + 1, X.shape[1]), max_iter=max_iter, tolerance=tolerance)
